...
class SpongeBotApplication(private val token: String) : ListenerAdapter() {
    ...
    override fun onReady(event: ReadyEvent) {
        ...
        event.jda.updateCommands().addCommands(
            Commands.slash("spongebot", "Create a mocking SpongeBob meme")
                .addOption(OptionType.STRING, "message", "The message to mock", true)
        ).queue()
    }
    ...
    override fun onSlashCommandInteraction(event: SlashCommandInteractionEvent) {
        if (event.name == "spongebot") {
            ...
            logger.info("Processing mocking meme request from user: $userName ($userId) with message: \"$message\"")
            ...
            try {
                ...
                val fileUpload = FileUpload.fromData(outputStream.toByteArray(), "spongebob_meme.png")
                ...
                event.hook.sendMessage("Sorry, I couldn't create the meme. Please try again.").queue()
            }
        }    }
    
    private fun createMockingMeme(text: String): BufferedImage {
        // Convert text to alternating case (mocking text)
        val mockingText = text.toCharArray().mapIndexed { index, char ->
        ...
        val (topText, bottomText) = if (mockingText.length > maxLineLength) {
            logger.debug("Text length (${mockingText.length}) exceeds max line length ($maxLineLength), splitting into two lines")
        ...
        val finalImage = BufferedImage(templateImage.width, templateImage.height, BufferedImage.TYPE_INT_RGB)
        ...
        val fontSize = if (topText.isNotEmpty() && bottomText.isNotEmpty()) {
        ...
        if (topText.isNotEmpty()) {
            drawTextWithOutline(graphics, topText, templateImage.width, 60, maxWidth, fontSize) // Top position
            logger.debug("Drew top text: \"$topText\" with font size $fontSize")
        }
        
        if (bottomText.isNotEmpty()) {
            drawTextWithOutline(graphics, bottomText, templateImage.width, templateImage.height - 50, maxWidth, fontSize) // Bottom position
    ...
    private fun splitTextIntoLines(text: String, maxLineLength: Int): Pair<String, String> {
        ...
        if (topWords.isEmpty() && bottomWords.isNotEmpty()) {
            ...
            return bottomWords.take(midPoint).joinToString(" ") to bottomWords.drop(midPoint).joinToString(" ")
        }
        
        return topWords.joinToString(" ") to bottomWords.joinToString(" ")
    }
      private fun drawTextWithOutline(graphics: Graphics2D, text: String, imageWidth: Int, yPosition: Int, maxWidth: Int, fontSize: Int) {
